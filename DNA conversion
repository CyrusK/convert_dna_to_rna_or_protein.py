#Convert DNA string into RNA translation or theoretically valid Protein optionally via the command line in 
#python by taking the filename and running either  code_name.py --rna dna.txt or code_name.py --genes dna.txt 
#Takes cDNA and converts it into RNA and AA sequence
...
#Convert cDNA to corresponding RNA sequence
...
#Now convert RNA sequence into AA via condon genetic code
# " *** " are the stop codons
...
def genes_operation(filename, start, cdna_triplet):
    ...
    for i in xrange (start, len(dna_string), 3):
       ...
       print protein_seq
       print                                                                            
       print "Here is your protein sequence(s) coded by Cyrus Khajvandi:"
